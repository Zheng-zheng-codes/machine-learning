...
for lr in lrs:
    ...
    model_relu=train_model(model_relu,lr)

    print("Training Tanh model...")
    model_tanh=train_model(model_tanh,lr)
    relu_acc_list.append(test_model(model_relu))
    tanh_acc_list.append(test_model(model_tanh))

plt.figure()

plt.plot(lrs, relu_acc_list, marker='o', label='ReLU')
plt.plot(lrs, tanh_acc_list, marker='s', label='Tanh')

plt.xscale('log')  # 学习率一般用log坐标更清晰
plt.xlabel('Learning Rate')
plt.ylabel('Accuracy')
plt.title('LR vs Accuracy (PCA=100)')
plt.legend()
plt.grid(True)
