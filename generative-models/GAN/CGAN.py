...
#   labels: [batch_size]
# 输出:
#   fake_images: [batch_size, 3, 32, 32]
class ConditionalGenerator(nn.Module):
    ...
    def forward(self, noise, labels):
        ...
        # label_embed: [batch_size, embed_dim]

        label_embed = label_embed.view(label_embed.size(0), self.embed_dim, 1, 1)
        # label_embed: [batch_size, embed_dim, 1, 1]

        x = torch.cat([noise, label_embed], dim=1)
        # x: [batch_size, noise_dim + embed_dim, 1, 1]
...
class ConditionalDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        # 把类别标签变成一张 32×32 的条件图
        self.label_embedding = nn.Embedding(
            num_classes,
            image_size * image_size
        )

        # 原图是 3 通道，标签条件图是 1 通道
        # 拼接后输入通道数变成 4
        input_channels = image_channels + 1

        self.main = nn.Sequential(
            # 输入: [batch_size, 4, 32, 32]
            # 输出: [batch_size, 64, 16, 16]
            nn.Conv2d(
    ...
    def forward(self, images, labels):
        ...
        # label_map: [batch_size, 1, 32, 32]

        x = torch.cat([images, label_map], dim=1)
        # x: [batch_size, 4, 32, 32]
