...
# 8. 开始训练
# =========================
print("Start training WGAN-GP on MNIST")
...
for epoch in range(1, num_epochs + 1):
    ...
    for batch_idx, (real_images, _) in enumerate(train_loader):
        ...
        # =========================
        if batch_idx % 100 == 0:
            if lossG_value is None:
                lossG_text = "not updated"
            else:
                lossG_text = f"{lossG_value:.4f}"
...
plt.ylabel("Loss")
plt.title("WGAN-GP Training Loss on MNIST")
plt.legend()
plt.grid(True)
